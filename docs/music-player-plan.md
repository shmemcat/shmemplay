...
Queue deletion removes only the saved queue. Deleting the active queue selects the preceding queue in the current picker order (queue 3 → queue 2), or the following queue when deleting the first. Resume that queue's saved song and position, preserving playing or paused state. An empty destination stays active without audio; deleting the only queue stops. Deleting an inactive queue leaves active playback alone. No source playlist or audio deletion is implied. Removing the currently playing song from its queue immediately advances playback to the next entry in playback order, rather than finishing the removed song. Removing any other entry leaves playback uninterrupted.
...
Source identity must survive supported replacement/rename workflows or request explicit repair. Read M3Us after in-app edits and on reopening/resuming, preloading membership for the add-track flow. Cache the song index across launches and rescan it through Settings after PC syncs or after in-app audio edits. Folder filtering relinks loaded playlist paths without reopening the files.
