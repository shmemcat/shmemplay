...
- Nested AND/OR membership/non-membership rules with Snapshot/Live output. Live items share the playlist category with All/Files/Live filters. Live definitions can be renamed, removed, and edited to repair source references. Missing/unreadable sources are errors, not empty sets. Creation rereads sources and rejects detected concurrent changes. Foreground/manual refresh and in-app M3U edits trigger reevaluation; unchanged file snapshots and unaffected live results are cached.
...
- Stop-after is consumed when that entry naturally ends, takes precedence over repeat, and does not affect copies in other queues. Active-queue deletion selects the previous picker queue (or the following queue when deleting the first), preserving the destination's saved song/position and playing/paused state. Deleting the only queue or selecting an empty destination stops audio.
...
## September 8 startup and queue follow-up

- Persist the song index in an atomic private `library-snapshot-v1.bin` file. Cold starts restore it without a MediaStore library query; missing/corrupt caches rebuild. Audio edits/deletion invalidate the index and successful in-app changes refresh it. Settings provides **Rescan library & playlists** after PC syncs.
- Continue reading M3Us on open/return so add-track membership is ready in advance. Remove the unconditional scan delay and centered playlist-scan spinner over the browser. The existing targeted write checks and verification remain intact.
- Use `Search - <query>` for queues created from search results, including selection menus and playlist shuffle. Active-queue deletion follows the preceding picker queue as described above.
- JVM regression tests cover saved-cache restore, explicit rescan, invalidation/corruption, folder relinking, queue order/boundaries, and playing/paused transitions. Build, lint, and JVM tests pass; phone startup timing and these UI flows still need device validation.

### Indexed search follow-up

- Build an immutable trigram index from normalized song metadata once per library/queue snapshot, on a background dispatcher. Queries normalize once, choose the rarest three-character fragment's posting list, and verify all full terms against those candidates. One/two-character terms search the already-normalized text; matching remains substring-based, accent/apostrophe tolerant, case insensitive, and AND across terms. Source order is preserved.
- Share one cancellable background projection between displayed rows, group/playlist counts, selection and queue creation. Precompute metadata groups, playlist name/path normalization and playlist entry order. Keep alphabet-scroll indexing off the UI thread. Do not convert every saved queue into library objects when no action needs them. Superseded results are not exposed under a newer query.
- `SubstringSearchIndexTest` compares the index with the former normalizer/matcher, exercises randomized and Unicode queries, verifies candidate narrowing and cancellation, and reports a reproducible 30,000-song synthetic JVM benchmark without timing-based pass/fail assertions. `BrowserSearchTest` covers song/group/playlist/queue projections, duplicate and missing playlist entries, source order and metadata replacement.
- Initial desktop matching-only measurements: former search 78–132 ms per query; indexed search 0.001–2.3 ms, with a one-time 112 ms index build. These are synthetic PC measurements, not phone UI latency or a comparison against Musicolet. Results and index memory costs depend on library contents; phone validation remains outstanding.
- Cached song rows publish immediately without waiting for either index stage. Album/artist/genre groups publish after their lightweight grouping pass, before trigram construction. Views that are genuinely still preparing show a loading state instead of a false empty-library message.
...
- The main title/subtitle has a larger left inset. Membership-header right padding is 34 dp. All back chevrons share a circular 40 dp press target with 12 dp outside spacing on each side, owned by one shared component so modal and detail headers stay consistent.
