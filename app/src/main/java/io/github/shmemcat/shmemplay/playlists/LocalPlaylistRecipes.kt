...
data class LocalPlaylistRecipe(val id: String, val name: String, val rule: PlaylistRuleNode, val live: Boolean = true)
...
class LocalPlaylistRecipes(context: Context) {
    ...
    fun load(): List<LocalPlaylistRecipe> {
        ...
        return List(array.length()) { index -> array.getJSONObject(index).let {
            LocalPlaylistRecipe(it.getString("id"), it.getString("name"), decodeRule(it.getJSONObject("rule")), it.optBoolean("live",true))
        } }
    }
    ...
    private fun write(recipes: List<LocalPlaylistRecipe>) {
        val array = JSONArray()
        recipes.forEach { array.put(JSONObject().put("id",it.id).put("name",it.name).put("live",it.live).put("rule",encodeRule(it.rule))) }
    ...
    companion object {
        ...
        fun evaluate(recipes: List<LocalPlaylistRecipe>, scan: PlaylistLibraryScan, tracks: List<LibraryTrack>): List<PlaylistSnapshot> {
            val membership = scan.playlists.associate { it.document.uri.toString() to it.resolvedTrackIds }
            val universe = tracks.mapTo(linkedSetOf()) { it.stableId }
            ...
            return recipes.filter { it.live }.map { recipe ->
                ...
                val result = runCatching { NestedPlaylistRules.evaluate(universe,membership,recipe.rule) }
