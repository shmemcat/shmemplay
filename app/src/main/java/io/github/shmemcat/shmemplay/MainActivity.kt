...
class MainActivity : ComponentActivity() {
    ...
    override fun onStart() {
        ...
        controller = future
        future.addListener({ runCatching { future.get() }.onFailure { if (!future.isCancelled) player.reportError("Could not connect to playback: ${it.message}") } }, ContextCompat.getMainExecutor(this))
    }
    override fun onStop() {
        player.checkpoint()
